...
int jpegd_prepare_hardware_info(motion_jpeg_obj *jpeg_hdl_ctx)
{
...

    {
        ...
        for (index = 0, i = 0; i < 16; i++) {
            ...
            for (j = 0; (j < num) && (index < 12); j++, index++) {
                ...
                huffman_table[pos] |= ((i + 1) << 20) + (((j + huf_tab->min_code[i]) << 12) & 0xFF000);
            }
        }
    }

    {
        ...
        for (index = 0, i = 0; i < 16; i++) {
            ...
            for (j = 0; (j < num) && (index < 12); j++, index++) {
                int pos = huf_tab->huffman_val[index];
                huffman_table[pos] |= ((i + 1) << 8) + ((j + huf_tab->min_code[i]) & 0xFF);
            }
        }
    }

    {
        ...
        for (i = 0; i < 16; i++) {
            ...
            luma_ac_min_table[i] = (huf_tab->min_code[i]) & 0xFF;
...

    {
        ...
        for (i = 0; i < 16; i++) {
            ...
            chroma_ac_min_table[i] = (huf_tab->min_code[i]) & 0xFF;
