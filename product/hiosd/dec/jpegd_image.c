...
int decode_dht(motion_jpeg_obj *mjpeg, const unsigned char *stream, unsigned int len)
{
    ...
    while (k < lh) {
        ...
        for (i = 0, tab_len = 0; i < 16; i++) {
            ...
            if (ptr[i]) {
                huffman->min_code[i] = (max_code) << 1;
                huffman->max_code[i] = huffman->min_code[i] + ptr[i] - 1;
                huffman->max_code16[i] = ((huffman->max_code[i] + 1) << (15 - i)) - 1;
                huffman->len[i] = ptr[i];
                huffman->huffman_val_ptr[i] = tab_len;
                max_code = huffman->max_code[i] + 1;
                error_flag |= ((huffman->max_code[i] + 1) > (1 << (i + 1)));
            } else {
                huffman->min_code[i] = max_code << 1;
...
int decode_sof0(motion_jpeg_obj *mjpeg, const unsigned char *stream, unsigned int len)  // 0xFF C0
{
    ...
    error_flag |= (header.color_depth != 8);
    error_flag |= (ls != (header.nf * 3 + 8));
    error_flag |= (header.y_height > mjpeg->max_height);
    error_flag |= (header.y_width > mjpeg->max_width);
    error_flag |= (header.y_width == 0);
    error_flag |= (header.y_height == 0);
    error_flag |= ((header.nf != 1) && (header.nf != 3));
    ...
    if (header.nf == 1) {
        ...
        error_flag |= (header.h[0] != 0x11);  // only support H = V = 1;
        error_flag |= (header.tq[0] & 0xfc);  // tq = (0 ~ 3), inclusive
        if (error_flag != 0) {
            return HI_UNSUPPORT_PIC_STRUCT;
        }
    ...
    if (header.nf == 3) {
        ...
        error_flag |= (header.h[1] != 0x11);  // only support H = V = 1;
        error_flag |= (header.h[2] != 0x11);  // only support H = V = 1;
        error_flag |= (header.tq[0] & 0xfc);  // tq = (0 ~ 3), inclusive
        error_flag |= (header.tq[1] & 0xfc);  // tq = (0 ~ 3), inclusive
        error_flag |= (header.tq[2] & 0xfc);  // tq = (0 ~ 3), inclusive
...
int decode_sos(motion_jpeg_obj *mjpeg, unsigned char *stream, unsigned int len)  // 0xFF DA
{
    ...
    error_flag |= (ns != fh->nf);  // ns should be equal to nf in frame header
    error_flag |= (ls != (ns * 2 + 6));
    ...
    for (i = 0; (i < ns) && (error_flag == 0); i++) {
        ...
        error_flag |= (scan.cs[i] != fh->ci[i]);
        error_flag |= (scan.td[i] > 1);  // baseline: td  = (0 ~ 1), inclusive
        error_flag |= (scan.ta[i] > 1);  // baseline: ta  = (0 ~ 1), inclusive
...
int decode_dqt(motion_jpeg_obj *mjpeg, const unsigned char *stream, unsigned int len)
{
    ...
    while ((k < lq) && (error_flag == 0)) {
        ...
        pq = code >> 4;
        tq = code & 0xf;
        error_flag |= (pq != 0);  // pq must be 0 in baseline
        error_flag |= (tq > 3);  // tq should be (0 ~ 3), inclusive
        error_flag |= (k > lq);
        if (error_flag) {
            return error_flag;
        }
